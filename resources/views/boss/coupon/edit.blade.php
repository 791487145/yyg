...
@section("content")
    <div id="all">
        <form id="form" action="/coupon/update/{{$id}}" method="post">
            <table class="table table-border table-bordered table-hover">
                <tr>
                    <td ><span class="input-padding">优惠券名称：</span><input type="text" value="{{$coupon->title}}"  datatype="*" class="input-css" name="title"></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td>
                        <span class="input-padding">使用条件：</span>一次性购满 &nbsp;&nbsp;<input style="width: 80px;" type="text" class="input-css"  value="{{$coupon->amount_order}}" name="amount_order" datatype="*" />&nbsp;&nbsp;元减&nbsp;&nbsp;<input type="text" style="width: 80px;" class="input-css"  value="{{$coupon->amount_coupon}}" name="amount_coupon" datatype="*" />&nbsp;&nbsp;元
                    </td>
                </tr>
                <tr>
                    <td>
                        <span class="inline">有效期
                <input type="text" name="start_time" datatype="*" value="{{$coupon->start_time}}"  value="" id="timeStart" class="input-text Wdate" style="width:180px;">
            -
            <input type="text" name="end_time" datatype="*" value="{{$coupon->end_time}}" id="timeEnd" class="input-text Wdate" style="width:180px;"></span>
                    </td>

                </tr>
                <tr>
                    <td ><span class="input-padding">优惠券状态：</span>
                        <select class="input-css" style="width: 130px" name="state">
                            <option value="1" @if($coupon->state == 1) selected="selected" @endif>正常使用</option>
                            <option value="2" @if($coupon->state == 2) selected="selected" @endif>暂停使用</option>
                        </select>
                    </td>
                </tr>
            </table>
...
@section("javascript")
    ...
    <script>
        ...
        $("#supplier-id").change(function(){ supplier_goods() });
        supplier_goods();

        function supplier_goods(){

            $(".inputOption").html('');
            $(".selectText").html('选择商品');
            var id = $("#supplier-id").val();
            if(id){
                $.post('/coupon/supplier/goods/'+id+'/<?php echo $id?>',function(data){
                    var item = ''
                    for(var i = 0; i<data.length;i++){
                        var checked = data[i].checked?'checked = "checked"':'';
                        item += '<p>' +
                                '<input class="selectCheckbox" '+checked+' type="checkbox" name="goods_id[]" value="'+data[i].id+'" data-title="'+data[i].title+'" name="goodsId[]"/>' +
